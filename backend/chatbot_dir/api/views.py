...
class IncorrectAnswerResponseView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Incorrect answer received"}, status=status.HTTP_200_OK
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            result = capture_interaction(
                ...
                incorrect_answer=serializer.validated_data.get(
...
class ViewSummaryView(APIView):
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "No interactions found"}, status=status.HTTP_404_NOT_FOUND
            )
