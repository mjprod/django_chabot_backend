...
class BrainManager:
    ...
    def load_knowledge_content_to_collection(self, collection_name):
       ...
       # add knowledge content from Brain table in DB
        try:
            knowledge_contents = self._fetch_knowledge_contents_from_db(collection_name)

            if knowledge_contents:
                self.add_documents(collection_name, knowledge_contents)
            else:
                logger.info("No knowledge contents found to load.")
        except Exception as e:
