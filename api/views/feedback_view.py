...
class CaptureFeedbackMultiView(MongoDBMixin, APIView):
    async def post(self, request):
        ...
        try:
            ...
            try:
                # Inicializar resultados de tradução
                translations = []

                with ThreadPoolExecutor() as executor:
                    # Lista de tarefas de tradução
                    tasks = []

                    # Verificar se a tradução é necessária para cada idioma
                    if language == "ms-MY" and generation != transformed_data.get("correct_answer", ""):
                        malay_future = executor.submit(translate_en_to_ms, generation)
                        tasks.append(asyncio.wrap_future(malay_future))
                    else:
                    # Se já está no idioma correto, adicione diretamente
                        translations.append({"language": "ms-MY", "text": transformed_data.get("correct_answer", "")})

                    if language == "cn" and generation != transformed_data.get("correct_answer", ""):
                        chinese_future = executor.submit(translate_en_to_cn, generation)
                        tasks.append(asyncio.wrap_future(chinese_future))
                    else:
                        translations.append({"language": "cn", "text": transformed_data.get("correct_answer", "")})

                    if language == "en" and generation != transformed_data.get("correct_answer", ""):
                        english_future = executor.submit(translate_and_clean, generation)
                        tasks.append(asyncio.wrap_future(english_future))
                    else:
                        translations.append({"language": "en", "text": transformed_data.get("correct_answer", "")})

                    # Executar as traduções assíncronas
                    logger.info("Submitted translation tasks...")
                    async_results = await asyncio.gather(*tasks, return_exceptions=True)

                    # Adicionar resultados assíncronos às traduções existentes
                    ...
                    # Log final e retorno estruturado
                    logger.info(f"Final translations: {translations}")
                    # return translations
