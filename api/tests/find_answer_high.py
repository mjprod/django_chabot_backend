...
def rephrase_correct_answer_dynamic(text):
    try:
        ...
        rephrased_options = [p["generated_text"] for p in paraphrases if "generated_text" in p]
...
def search_top_answer_and_translate(query, collection_name="feedback_data"):
    ...
    try:
        ...
        if results_list:
            ...
            print(Fore.BLUE + f"- Paraphrased Answer (EN): {rephrased_answer}" + Style.RESET_ALL)
